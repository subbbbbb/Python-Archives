...
def getTax(salary):
    taxRate = getTaxRate()
    tax = salary*taxRate
    print("\nKeep in mind, due to a variety of variables, your income tax will vary, depending on state legislature, this provides an average estimate")
...
def main():
    salary = getSalary()
    tax = getTax(salary)
    print("\nLet's visualize your spending!\nWe will separate your basic budget into 5 categories - Housing, Food, Transportation, Bills/Utilities/Health Care, Leisure")
    ...
    savings = (salary) - (housing + transportation + bills + lesiure + tax)
    
    labels = "Housing", "Transportation", "Bills", "Leisure", "Savings", "Tax"
    sizes = [housing, transportation, bills, lesiure, savings, tax]
    ...
    if (housing + transportation + bills + lesiure + tax < salary):
